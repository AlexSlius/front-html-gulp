...
const paths = {
	entry: {
		...
		all: `${sourse_folder}libs/**/*`,
		sass: `${sourse_folder}styles/**/*.scss`,
...
function TScriptsAll() {
	return src([paths.entry.all])
		.pipe(dest(paths.out.libs))
		.pipe(MbrowserSync.stream())
}
...
function TstartWatch() {
	...
	watch([paths.entry.script], TScriptsAll);
	watch([paths.entry.svg], TSpriteIcon);
...
exports.image = TImages;
exports.fonts = series(TFontfConverTowoff, TFontfConverTowoff2);
exports.build = series(THtml, TScripts, TScriptsAll, TSass, TFontfConverTowoff, TFontfConverTowoff2, TImages);
exports.dev = series(THtml, TSpriteIcon, TImages, TScripts, TScriptsAll, TSass, parallel(Tbrowsersync, TstartWatch));